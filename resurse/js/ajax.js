window.onload=function(){
	...
	*/
	...
	ajaxRequest.open("GET", "json/infractori.json", true);
    ...
    function afiseajaJsonTemplate(obJson) { 
			...
			for(let i=0;i<obJson.infractori.length;i++){
				...
				textTemplate+=ejs.render("<div class='templ_infractor'>\
                    <p>Id: <%= infractor.id %></p>\
                    <p>Grupa: <%= infractor.grupa %></p>\
                    <p>Nume complet: <%= infractor.nume %></p>\
                    <p>Descriere infractiune: <%= infractor.descriere_infractiune %></p>\
                    <p>Numar victime: <%= infractor.numar_victime %></p>\
                    <p>Este in libertate: <%= infractor.liber %></p>\
                    <p>Semnalmente: <%= infractor.semnalmente %></p>\
                    <p>Data infractiunii: <%= infractor.data_infractiune %></p>\
				</div>", 
				{infractor: obJson.infractori[i]});
			} 
